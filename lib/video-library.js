...
window.fsVL = (function () {
  ...
  // ── the catalogue cache ─────────────────────────────────────────────────
  // The catalogue is the same for every member, so navigating the library
  // should not refetch the world per page. Raw rows live in sessionStorage
  // (like the thumb tokens) keyed on the app version; a hit paints
  // instantly and a background refetch rewrites the cache, so it is never
  // more than one navigation stale. Personal layers (progress, saves) are
  // deliberately NOT cached — Continue watching must always be current.
  var CAT_KEY = 'fs-vl-catalogue';
  var CAT_TTL = 5 * 60 * 1000;
  function readCatCache() {
    try {
      var c = JSON.parse(sessionStorage.getItem(CAT_KEY));
      if (c && c.v === (window.fsVersion || '') && Date.now() - c.t < CAT_TTL) return c.raw;
    } catch (_) {}
    return null;
  }
  function writeCatCache(raw) {
    try {
      sessionStorage.setItem(CAT_KEY, JSON.stringify({
        t: Date.now(), v: window.fsVersion || '', raw: raw }));
    } catch (_) {}   // storage full or blocked — the cache is a luxury
    return raw;
  }

  async function loadCatalogue() {
    var cached = readCatCache();
    if (cached) {
      fetchCatalogue().then(writeCatCache).catch(function () {});
      return buildCatalogue(cached);
    }
    return buildCatalogue(writeCatCache(await fetchCatalogue()));
  }
  ...
  async function fetchCatalogue() {
    ...
    var raw = {
      ...
      statsRows: got[3].data || [],
      vlinks: got[4].data || [], clinks: got[5].data || [],
      filters: got[6].data || [], options: got[7].data || [],
      values: got[8].data || [], items: got[9].data || [],
      instructorRows: got[10].data || [],
      featuredRows: [],
    };
    ...
    try {
      ...
      if (!fr.error) raw.featuredRows = fr.data || [];
    } catch (_) {}
    return raw;
  }

  // raw rows → the catalogue with its lookup maps (maps never enter the
  // cache; they are rebuilt from rows on every load, cached or fresh)
  function buildCatalogue(raw) {
    var L = {
      categories: raw.categories, videos: raw.videos, collections: raw.collections,
      stats: {}, vlinks: raw.vlinks, clinks: raw.clinks,
      filters: raw.filters, options: raw.options,
      values: raw.values, items: raw.items,
      featuredRows: raw.featuredRows || [],
      instructors: {}, byVideo: {}, byCollection: {},
    };
    raw.statsRows.forEach(function (s) { L.stats[s.collection_id] = s; });
    raw.instructorRows.forEach(function (i) { L.instructors[i.id] = i.name; });
  ...
  function collectionItem(L, c) {
    ...
    // the REAL videos behind the cover, for the card's stack-of-images cue.
    // When the collection wears its own artwork the stack starts at video 1;
    // when the cover already IS video 1, the stack starts at video 2. Depth
    // is honest: a two-video collection shows one sheet, a lone video none.
    var inColl = L.items.filter(function (i) {
      return i.collection_id === c.id && i.kind === 'video' && L.byVideo[i.video_id];
    }).sort(function (a, b) { return a.position - b.position; })
      .map(function (i) { return L.byVideo[i.video_id]; });
    var ownCover = !!(c.thumbnail_mode === 'custom' && c.custom_thumbnail_url);
    var stack = inColl.slice(ownCover ? 0 : 1, ownCover ? 2 : 3).map(function (v) {
      var va = (v.thumbnail_mode === 'custom' && v.custom_thumbnail_url) ? v.custom_thumbnail_url : null;
      return { art: va, playbackId: va ? null : (v.mux_playback_id || null), videoId: v.id };
    });
    return {
      stack: stack,
  ...
  function cardHTML(item) {
    ...
    // a started video wears a thin progress line along the bottom of its
    // artwork — quiet on the shelf, load-bearing on Continue watching
    var prog = '';
    if (item.kind === 'video') {
      var p = progressOf(item.id);
      if (progressStarted(p) && Number(p.duration_seconds)) {
        var pct = Math.min(96, Math.round(p.position_seconds / p.duration_seconds * 100));
        prog = '<span class="vcard-progress"><span style="width:' + pct + '%"></span></span>';
      }
    }
    // collections are a deck of REAL images: the sheets behind the cover are
    // actual thumbnails from inside, farthest first so nearer paints over.
    // A sheet with no art yet hydrates from its signed still, or stays the
    // quiet tinted edge the old cue used.
    var deck = '';
    if (item.kind === 'collection') {
      deck = (item.stack || []).map(function (s2, i) {
        var attrs = (!s2.art && s2.playbackId)
          ? ' data-tok-video="' + esc(s2.videoId) + '" data-playback="' + esc(s2.playbackId) + '" data-w="320"'
          : '';
        return '<span class="vcard-stack vs-' + (i + 1) + '"' + attrs + '>' +
          (s2.art ? '<img alt="" loading="lazy" src="' + esc(s2.art) + '">' : '') + '</span>';
      }).reverse().join('');
    }
    var artBody = item.kind === 'collection'
      ? deck + '<span class="vcard-art-in"' + a[1] + '>' + a[0] + '</span>'
      : a[0] +
  ...
  async function saveCollection(collectionId) {
    var r = await db.from('library_saves')
      .insert({ member_id: userId(), collection_id: collectionId }).select().single();
    if (r.error && !/duplicate/i.test(r.error.message || '')) throw r.error;
    return r.data;
  }
  async function unsaveCollection(collectionId) {
    var r = await db.from('library_saves').delete()
      .eq('member_id', userId()).eq('collection_id', collectionId);
    if (r.error) throw r.error;
  }
  // the Saved shelf: everything this member bookmarked, newest first,
  // resolved against the catalogue. (An unlisted collection stays out —
  // it lives at its link, never on a browse surface, saved or not.)
  async function savedItems(L) {
    try {
      var r = await db.from('library_saves')
        .select('video_id, collection_id, created_at')
        .order('created_at', { ascending: false });
      if (r.error) return [];
      return (r.data || []).map(function (s) {
        if (s.video_id && L.byVideo[s.video_id]) return videoItem(L, L.byVideo[s.video_id]);
        if (s.collection_id && L.byCollection[s.collection_id]) return collectionItem(L, L.byCollection[s.collection_id]);
        return null;
      }).filter(Boolean);
    } catch (_) { return []; }   // engagement SQL not run yet — the shelf hides
  }

  // ── watch progress ──────────────────────────────────────────────────────
  // One member-owned row per video (video-watch-progress.sql). Loaded once
  // per page into a map that cardHTML and the playlists read; if the table
  // has not been created yet, every path below fails quietly and the
  // feature simply is not there.
  var PROG = null;
  // force: true refetches — pages restored from the back/forward cache call
  // this so the personal layer is never yesterday's
  async function loadProgress(force) {
    if (PROG && !force) return PROG;
    PROG = {};
    try {
      var r = await db.from('video_watch_progress')
        .select('video_id, position_seconds, duration_seconds, completed, updated_at');
      if (!r.error) (r.data || []).forEach(function (p) { PROG[p.video_id] = p; });
    } catch (_) {}
    return PROG;
  }
  function progressOf(videoId) { return (PROG || {})[videoId] || null; }
  // "started" = worth resuming: past the first moments, not essentially done
  function progressStarted(p) {
    if (!p || p.completed) return false;
    var pos = Number(p.position_seconds) || 0;
    var dur = Number(p.duration_seconds) || 0;
    return pos >= 15 && (!dur || pos < dur * 0.95);
  }
  async function saveProgress(videoId, position, duration, completed) {
    try {
      await db.from('video_watch_progress').upsert({
        member_id: userId(),
        video_id: videoId,
        position_seconds: Math.max(0, Math.round(position || 0)),
        duration_seconds: duration || null,
        completed: !!completed,
        updated_at: new Date().toISOString(),
      });
      if (PROG) PROG[videoId] = { video_id: videoId, position_seconds: position,
        duration_seconds: duration, completed: !!completed };
    } catch (_) {}   // progress must never interrupt watching
  }
  // the Continue watching shelf: my in-flight videos, newest activity first
  function continueWatching(L, cap) {
    if (!PROG) return [];
    return Object.keys(PROG)
      .map(function (id) { return PROG[id]; })
      .filter(function (p) { return progressStarted(p) && L.byVideo[p.video_id]; })
      .sort(function (a, b) { return new Date(b.updated_at) - new Date(a.updated_at); })
      .slice(0, cap || 12)
      .map(function (p) { return videoItem(L, L.byVideo[p.video_id]); });
  }

  // ── rich descriptions ───────────────────────────────────────────────────
  // The admin's description editor writes a small dialect of HTML — p, br,
  // strong, em, u, ul/ol/li, a[href]. This renders that dialect and NOTHING
  // else: the markup is walked and rebuilt from the allowlist, links are
  // scheme-checked and open in a new tab. Legacy plain-text descriptions
  // (no tags) come back escaped with their line breaks kept.
  function richText(src) {
    ...
    function safeHref(h) {
      h = (h || '').trim();
      return /^(https?:|mailto:)/i.test(h) || /^\//.test(h) ? h : null;
    }
    function ser(node) {
      if (node.nodeType === 3) return esc(node.nodeValue);
      if (node.nodeType !== 1) return '';
      var kids = Array.prototype.map.call(node.childNodes, ser).join('');
      switch (node.tagName) {
        case 'BR': return '<br>';
        case 'B': case 'STRONG': return kids && '<strong>' + kids + '</strong>';
        case 'I': case 'EM':     return kids && '<em>' + kids + '</em>';
        case 'U':                return kids && '<u>' + kids + '</u>';
        case 'UL': case 'OL':    return kids && '<' + node.tagName.toLowerCase() + '>' + kids + '</' + node.tagName.toLowerCase() + '>';
        case 'LI':               return '<li>' + kids + '</li>';
        case 'A': {
          var h = safeHref(node.getAttribute('href'));
          return h ? '<a href="' + esc(h) + '" target="_blank" rel="noopener">' + kids + '</a>' : kids;
        }
        case 'P': case 'DIV':    return kids ? '<p>' + kids + '</p>' : '';
    ...
    box.innerHTML = src;
    return Array.prototype.map.call(box.childNodes, ser).join('');
  }
  ...
  return {
    ...
    richText: richText,
    loadProgress: loadProgress, progressOf: progressOf,
    progressStarted: progressStarted, saveProgress: saveProgress,
    ...
    saveCollection: saveCollection, unsaveCollection: unsaveCollection,
