...
window.fsVL = (function () {
  ...
  function featured(L, cap) {
    return (L.featuredRows || []).map(function (f) {
      if (f.video_id && L.byVideo[f.video_id]) return videoItem(L, L.byVideo[f.video_id]);
      if (f.collection_id && L.byCollection[f.collection_id]) return collectionItem(L, L.byCollection[f.collection_id]);
      return null;
