...
window.fsPublish = (function () {
  ...
  const PROJ_SLOTS = { begin: 'Begin', warmup: 'Warm Up Flow', bridge: 'Bridge Flow',
    nohands: 'No Hands Flow', peak: 'Peak Flow', workshop: 'Workshop',
    slomo: 'Slo Mo Flow', restorative: 'Restorative Flow', cooldown: 'Cool Down' };
  ...
  function orderedFor(c) {
    // classes saved by the modular editor carry their own order; legacy
    // classes keep the historical template until their next save
    // custom section types (the teacher's flow types) resolve HERE, at
    // publish time — the label is stamped into the payload, so a Circle
    // viewer never needs the author's type library
    const mine = readLS('flowschool_flow_types');
    const slotLabel = t => PROJ_SLOTS[t] ||
      ((mine.find(x => x.key === t) || {}).name) || 'Workshop';
    if (c && Array.isArray(c.layout)) {
      return c.layout.map(k => {
        const std = PROJ_SECTIONS.find(s => s.key === k);
        if (std) return std;
        const x = ((c && c.extras) || []).find(e => e.key === k);
        return x ? { key: k, label: slotLabel(x.type) } : null;
      }).filter(Boolean);
    }
    const out = [];
    PROJ_SECTIONS.forEach(s => {
      ...
      if (s.key === 'nohands') {
        ((c && c.extras) || []).forEach(x => {
          out.push({ key: x.key, label: slotLabel(x.type) });
        });
      }
    });
    return out;
  ...
  function buildProjection(c) {
    ...
    const sections = orderedFor(c).map(s => {
      ...
      if (flow) {
        // the flow's own video link (a public URL the teacher chose to attach)
        if ((flow.videoUrl || '').trim()) out.video = flow.videoUrl.trim();
        const cues = sheets
  ...
  async function save(publicId) {
    const uid = await me();
    if (!uid) return { error: { message: 'not signed in' } };
    const res = await db.from('class_saves').upsert(
      { user_id: uid, public_class_id: publicId },
      { onConflict: 'user_id,public_class_id' });
    if (res && !res.error && window.fsTrack) fsTrack('public_item_saved', { resourceType: 'public_class', resourceId: publicId });
    return res;
  }
  ...
  async function unsave(publicId) {
    const uid = await me();
    if (!uid) return { error: { message: 'not signed in' } };
    const res = await db.from('class_saves')
      .delete().eq('user_id', uid).eq('public_class_id', publicId);
    if (res && !res.error && window.fsTrack) fsTrack('public_item_unsaved', { resourceType: 'public_class', resourceId: publicId });
    return res;
  }
  ...
  async function savedClasses() {
    ...
    // and carry when each save happened
    const byId = {};
    (data || []).forEach(function (r) { byId[r.id] = r; });
    const savedAt = {};
    (refs || []).forEach(function (r) { savedAt[r.public_class_id] = r.created_at; });
    return {
      data: ids.map(function (id) {
        return byId[id] ? Object.assign({ saved_at: savedAt[id] || null }, byId[id]) : null;
      }).filter(Boolean),
      error: null,
    };
  }
