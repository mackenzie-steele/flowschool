...
var SLOT_TYPES = {
  ...
  warmup:   { label: 'Warm Up Flow',  placeholder: 'QPED / BIRD DOG / SL CHILD / COBRA / …', rows: 2 },
  ...
  peak:     { label: 'Peak Flow',     placeholder: 'W2 / HANDS HEART SKAN / REV CL / HM / …', rows: 3 },
  workshop: { label: 'Workshop',      placeholder: 'Demo the step up transition', rows: 1 },
  cooldown: { label: 'Cool Down',     placeholder: 'PIGEON / JANU / SUPINE TWIST / …', rows: 2 },
};
var MAX_EXTRAS = 12;
...
function slotDef(type) {
  ...
  if (c) return { label: c.name, placeholder: 'Type your flow here…', rows: 2 };
...
var SLOT_PICK = (function () {
  ...
  // a class opens once: with a Begin seated (standing key or slot),
  // the option simply isn't offered
  function hasBegin() {
    return editLayout.indexOf('begin') !== -1 ||
      editExtras.some(function (x) {
        return x.type === 'begin' && editLayout.indexOf(x.key) !== -1;
      });
  }
