...
(function () {
  ...
  var NAV = [
    ...
    // Play runs body-first: get on the mat, then add a constraint, then
    // solve a path, then score it. Same order the Start Here doors offer.
    { id: 'movement-experiments', href: 'movement-experiments',          icon: 'timer',           label: 'Movement Experiments', group: 'Play' },
  ...
  var PAGE_IDS = {
    ...
    'saved.html':                         'saved',
    'admin.html':                         'admin',
    // the admin-only member-library preview (injected by fsNavAddAdmin);
    // slug routes (/video/x …) intentionally resolve to no active item
    'library.html':                       'platform-library',
  };
  ...
  // ── Admin link (added post-injection, only for admins) ────────────────────
  // Called by sync.js after a server-truth is_admin() check. It sits in the
  // account cluster at the bottom, beside the settings gear — Admin is a
  // platform/account destination, not a teaching tool. Convenience only:
  // /admin is protected server-side regardless of whether this renders.
  var navIsAdmin = false;

  function adminButtonHTML(cls, id) {
    var on = currentPageId() === 'admin';
    return '<a class="' + cls + '" id="' + id + '" href="admin" title="Admin" aria-label="Admin"'
      + (on ? ' aria-current="page"' : '') + '><span class="material-symbols-sharp">widgets</span></a>';
  }

  // ── Platform Library (admin-only nav row) ─────────────────────────────────
  // The member Video Library is admins-only while Bonnie QAs it, so its nav
  // row arrives the same way the Admin button does — injected after the
  // server-truth is_admin() check, never rendered for members. The lock is
  // the label's honesty: this door is locked for everyone else. It sits
  // directly ABOVE the (Uscreen) Video Library row it will eventually
  // replace. Convenience only — /library itself gates access regardless.
  var USCREEN_HREF = 'https://flowschool.uscreen.io';

  function injectPlatformLibrary() {
    try {
      var on = currentPageId() === 'platform-library';
      var rail = document.querySelector('a.nav-item[href="' + USCREEN_HREF + '"]');
      if (rail && !document.getElementById('nav-platform-lib')) {
        rail.insertAdjacentHTML('beforebegin',
          '<a href="library" class="nav-item' + (on ? ' active' : '') + '" id="nav-platform-lib"' +
            (on ? ' aria-current="page"' : '') + '>' +
            '<span class="material-symbols-sharp">lock</span>Platform Library' +
            (on ? '<span class="nav-dot"></span>' : '') +
          '</a>');
      }
      var sheet = document.querySelector('a.ns-row[href="' + USCREEN_HREF + '"]');
      if (sheet && !document.getElementById('nav-platform-lib-m')) {
        sheet.insertAdjacentHTML('beforebegin',
          '<a href="library" class="ns-row' + (on ? ' on' : '') + '" id="nav-platform-lib-m">' +
            '<span class="material-symbols-sharp">lock</span>' +
            '<span class="ns-label">Platform Library</span>' +
  ...
  function injectAdminButtons() {
    injectPlatformLibrary();
    try {
      var hdr = document.getElementById('hdr-auth');
      if (hdr && !document.getElementById('nav-admin-link')) {
        // The GEAR specifically — the bell wears .nav-settings-btn too (to
        // look identical to it), so a bare class lookup returned whichever
        // sat first and the admin button landed on a different side of the
        // bell depending on which had mounted. Same shape the mobile branch
        // below already uses.
        var gear = hdr.querySelector('a.nav-settings-btn[href="settings"]');
        var html = adminButtonHTML('nav-settings-btn nav-admin-btn', 'nav-admin-link');
        if (gear) gear.insertAdjacentHTML('beforebegin', html); else hdr.insertAdjacentHTML('beforeend', html);
      }
      var sheet = document.getElementById('ns-identity');
      if (sheet && sheet.querySelector('.ns-person') && !document.getElementById('nav-admin-link-m')) {
        var gearM = sheet.querySelector('a.ns-feedback-btn[href="settings"]');
        var htmlM = adminButtonHTML('ns-feedback-btn nav-admin-btn', 'nav-admin-link-m');
        if (gearM) gearM.insertAdjacentHTML('beforebegin', htmlM); else sheet.insertAdjacentHTML('beforeend', htmlM);
      }
    } catch (_) {}
  }

  window.fsNavAddAdmin = function () { navIsAdmin = true; injectAdminButtons(); };
  ...
  function renderSheetIdentity() {
    ...
    el.innerHTML =
      ...
      + '</button>';
    if (navIsAdmin) injectAdminButtons();
    if (window.fsNotifyMount) window.fsNotifyMount();
  }
  ...
  function injectFeedback() {
    ...
    modal.innerHTML =
      ...
      + '<h3 class="fb-title" id="fb-title">' + FB_TITLES.love + '</h3>'
      + '<div class="fb-chips" id="fb-chips">'
      +   '<button type="button" class="fb-chip on" data-kind="love"><span class="fb-chip-dot"></span>Love note</button>'
      +   '<button type="button" class="fb-chip" data-kind="idea"><span class="fb-chip-dot"></span>Idea</button>'
      +   '<button type="button" class="fb-chip" data-kind="bug"><span class="fb-chip-dot"></span>Bug</button>'
      + '</div>'
      + '<textarea class="fb-text" id="fb-text" rows="5" placeholder="' + FB_PLACEHOLDERS.love + '"></textarea>'
      + '<div class="fb-attach" id="fb-attach" hidden>'
    ...
    function setKind(kind) {
      ...
      // a love note doesn't need a screenshot — hide the attach affordance and
      // drop any shot already picked, so switching to love never sends one
      var attach = document.getElementById('fb-attach');
      if (attach) attach.hidden = (kind === 'love');
      if (kind === 'love') setShot(null);
  ...
  function openFeedback(kind) {
    ...
    else if (fbSetKind) fbSetKind('love');   // a fresh open starts on Love note

    // the page, version, device, etc. still attach silently to the payload;
    // we just no longer surface them (or the reply-to line) in the modal
  ...
  function injectSidebar() {
    ...
    // keepFocus: the sheet is closing to hand off to something else that has
    // already taken focus (the notification panel). Pulling focus back to the
    // hamburger here would yank it straight out of whatever just opened.
    function closeSheet(keepFocus) {
      sheet.classList.remove('open');
      overlay.classList.remove('show');
      unlockScroll();
      toggleBtn.setAttribute('aria-label', 'Open navigation');
      if (!keepFocus) toggleBtn.focus({ preventScroll: true });
    ...
    sheet.addEventListener('click', function (e) {
      // The bell wears .ns-feedback-btn so it matches its neighbours visually
      // — but that class also MEANS "open feedback" here. Without this guard a
      // tap on the bell opened the feedback modal and the notification panel
      // at once, the modal on top. Styling and behaviour shared one name.
      // The bell's own handler has already opened the panel by the time this
      // bubbles up. Drop the sheet out from under it — otherwise the panel
      // renders on top of the menu — but leave focus where the panel put it.
      if (e.target.closest('.fs-bell')) { closeSheet(true); return; }
  ...
  function renderAuth() {
    ...
    if (session && session.user) {
      ...
      el.innerHTML =
        ...
        + '</a>';
      if (navIsAdmin) injectAdminButtons();
      // the bell mounts itself beside the gear; it lives in its own file so a
      // fault there costs a bell, not the whole rail
      if (window.fsNotifyMount) window.fsNotifyMount();
    } else {
      el.innerHTML =
  ...
  // ── Search fields — one clear control, everywhere ─────────────────────────
  // Three behaviours had grown up side by side: the house × (Pose Library,
  // Your Flows), the BROWSER's × on type="search" (The Circle, Your Stories,
  // Your Classes), and nothing at all (Saved, Teaching Notes, Your Cues,
  // Admin). global.css now suppresses the native one; this puts the house one
  // on every search field that hasn't already got its own.
  //
  // Clearing dispatches a real `input` event, so each page's existing search
  // handler runs untouched — debounces, filters and counters all behave as if
  // the field had been emptied by hand. No page needs to know this happened,
  // which is the point: the next search bar someone adds is correct by
  // default rather than by memory.

  function upgradeSearch(root) {
    var wraps = (root || document).querySelectorAll('.search-wrap');
    [].forEach.call(wraps, function (wrap) {
      var input = wrap.querySelector('.search-input');
      if (!input) return;
      if (wrap.querySelector('.search-clear')) return;   // hand-rolled one already here
      if (input.dataset.fsClear) return;
      input.dataset.fsClear = '1';

      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'search-clear' + (input.value ? '' : ' hidden');
      ...
      btn.addEventListener('click', function () {
        ...
        // a real event, not a callback — the page's own handler owns what
        // "cleared" means, and this way it can't fall out of step with typing
  ...
  // Admin builds its search bars in innerHTML long after load, and library
  // pages re-render theirs, so a one-shot pass at boot would miss them.
  // Idempotent (data-fsClear), so re-scanning costs nothing.
  function watchSearch() {
    upgradeSearch(document);
    if (!window.MutationObserver || !document.body) return;
    var queued = false;
    new MutationObserver(function () {
      if (queued) return;
      queued = true;
      requestAnimationFrame(function () { queued = false; upgradeSearch(document); });
    }).observe(document.body, { childList: true, subtree: true });
  }

  // ── Site footer ───────────────────────────────────────────────────────────

  var FOOTER_MAIN = 'Flow School \u00B7 The Creative Studio for Yoga Teachers \u00B7 ' + VERSION;
  var FOOTER_SUPPORT = '<a href="mailto:support@flowschool.io">support@flowschool.io</a>';

  function injectFooter() {
    var footer = document.createElement('footer');
    footer.className = 'site-footer';
    footer.innerHTML = '<span class="footer-main">' + FOOTER_MAIN + '</span>'
      + '<span class="footer-support">' + FOOTER_SUPPORT + '</span>';
  ...
  function init() {
    ...
    renderAuth();
    renderSheetIdentity();
    // the footer needs the full document parsed (it appends to <main>), and
    // so do the search fields — this file runs before the page's own markup
    // exists, so a boot-time scan would find nothing to upgrade
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', function () { injectFooter(); watchSearch(); });
    } else {
      injectFooter();
      watchSearch();
    }
