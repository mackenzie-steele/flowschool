...
(function () {
  ...
  function buildSheetHTML(activeId) {
    ...
    // the standalone places (Dashboard keeps the dock below)
    NAV.forEach(function (item) {
      if (item.group || item.id === 'dashboard') return;
      var on = activeId === item.id;
      html += '<a href="' + item.href + '" class="ns-row' + (on ? ' on' : '') + '"' + (item.external ? ' target="_blank" rel="noopener"' : '') + '>'
