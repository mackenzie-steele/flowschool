...
(function () {
  ...
  function loadSdk() {
    ...
    sdkPromise = new Promise(function (resolve, reject) {
      ...
      s.src = SDK;
      s.async = true;
      // v3 fires this once MusicKit is ready to configure
      ...
      s.onerror = function () {
        ...
        reject(new Error('Could not reach Apple Music'));
      };
  ...
  function exportPlaylist(name, description, tracks) {
    ...
    return ready().then(function (music) {
      // authorize() opens Apple's sign-in — it must be inside the click that
      // started this, or the popup blocker eats it
  ...
  function explain(err) {
    ...
    if (/Could not reach Apple Music/.test(m)) return 'Couldn’t load Apple Music. Check your connection.';
