...
(function () {
  ...
  function findLibraryPlaylist(name) {
    return connect().then(function (music) {
      ...
      return page(0).then(function () {
        ...
        // canEdit:false is returned but NOT trusted as a veto. It is a hint,
        // and refusing on it meant the write never ran — so the only thing
        // that could give a real answer never got asked. Apple's response to
        // the actual POST is the authority; this just travels with the result
        // so the caller can say it tried against Apple's own advice.
  ...
  function explain(err) {
    ...
    // Apple's own bug, documented on their forums: adding tracks works for
    // every library playlist EXCEPT a collaborative one, which fails with
    // exactly this. Worth naming, because nothing about "Upstream Service
    // Error" suggests "turn off collaboration".
    if (/Unable to update tracks/i.test(m)) {
      return 'Apple refused the update. This is a known Apple bug with COLLABORATIVE playlists — '
        + 'open the playlist in Apple Music, turn collaboration off, then re-share it and paste the new link.';
    }
      ...
      return music.api.music('/v1/me/library/playlists', { limit: 100 }).then(function (res) {
        ...
        return {
          ...
          playlists: d.map(function (pl) {
            var a = pl.attributes || {};
            // everything, not a chosen few — the attribute that explains a
            // failure is never the one you thought to pick out in advance
            return {
              id: pl.id, name: a.name, canEdit: a.canEdit,
              isPublic: a.isPublic, hasCatalog: a.hasCatalog,
              attributes: a,
            };
          }),
        };
