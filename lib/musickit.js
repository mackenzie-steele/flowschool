...
(function () {
      ...
      function page(offset) {
        return music.api.music('/v1/me/library/playlists', { limit: 100, offset: offset })
          .then(function (res) {
            ...
            if (hit) {
              // a playlist added from the catalogue rather than created by
              // you is in your library and is NOT editable — worth saying so
              // rather than letting the write fail obscurely
              var canEdit = !(hit.attributes && hit.attributes.canEdit === false);
              return { music: music, id: hit.id, canEdit: canEdit,
  ...
  function explain(err) {
    ...
    if (m === 'NOT_EDITABLE') return 'That playlist isn’t editable from here — Apple only allows writes to playlists you created.';
