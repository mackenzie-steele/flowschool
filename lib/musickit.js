...
(function () {
  ...
  function addToPlaylist(name, tracks) {
    ...
    return findLibraryPlaylist(name).then(function (out) {
      ...
      var added = 0;
      // sequential, same as creation — Apple keeps playlist order and
      // concurrent appends interleave
      return chunks.reduce(function (chain, batch) {
        return chain.then(function () {
          return out.music.api.music('/v1/me/library/playlists/' + out.id + '/tracks', {}, {
            fetchOptions: {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
          ...
          }).then(function () { added += batch.length; });
        });
      }, Promise.resolve()).then(function () { return { added: added, playlistId: out.id }; });
    });
  }
