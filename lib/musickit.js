...
(function () {
    ...
    return connect().then(function (music) {
      ...
      function page(offset) {
        return music.api.music('/v1/me/library/playlists', { limit: 100, offset: offset })
          .then(function (res) {
            var d = (res && res.data) || {};
            var hit = (d.data || []).filter(function (pl) {
              return ((pl.attributes && pl.attributes.name) || '').trim().toLowerCase() === want;
            })[0];
            if (hit) return { music: music, id: hit.id };
  ...
  // Takes a LIBRARY id, because the caller has usually just looked it up and
  ...
  function addToPlaylistById(libraryId, tracks, onProgress) {
    ...
    return connect().then(function (music) {
      ...
      // interleave, and the order is the whole point of an energy arc
      return chunks.reduce(function (chain, batch) {
        return chain.then(function () {
          return music.api.music('/v1/me/library/playlists/' + libraryId + '/tracks', {}, {
            fetchOptions: {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
            ...
            // MusicKit resolves rather than throws on some API errors, so a
            // silent no-op looks identical to success unless we look
            if (res && res.data && res.data.errors && res.data.errors.length) {
              var e0 = res.data.errors[0];
              var err = new Error(e0.detail || e0.title || 'Apple refused the tracks');
              err.status = e0.status;
              throw err;
            }
            added += batch.length;
  ...
  function explain(err) {
    ...
    if (m === 'PLAYLIST_MISSING') return 'Couldn’t find that playlist in your Apple Music library — has it been renamed?';
